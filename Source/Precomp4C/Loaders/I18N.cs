...
namespace Precomp4C.Loaders {
    static class I18N {
        static readonly List<I18NItem> I18NItems = new();
        static readonly List<I18NLang> I18NLangs = new();
        static FileStream OutputHead = null;
        static FileStream OutputSource = null;
        public static Dictionary<string, string> FontDB = new()
        {
            { "ar", "Segoe UI" },
            { "el", "Segoe UI" },
            { "zh-Hans", "微软雅黑" },
        ...
        public class I18NLang {
            ...
            public List<KeyValuePair<int, string>> ItemTable = new();
            public I18NLang(CultureInfo CI) {
                Info = CI;
                Default = false;
                LangSymbol = "Precomp4C_I18N_Lang_" + Info.Name.ToString().Replace('-', '_');
            ...
            public void Output() {
                ...
                RTL.WriteOutput(OutputHead, "extern " + SymbolDef + ";");
                RTL.WriteOutput(OutputSource, SymbolDef + " = {");
                I18NItems.ForEach(GItem => {
                    ...
                    if (GItem != I18NItems[^1])
                        ItemDef += ',';
                    RTL.WriteOutput(OutputSource, ItemDef);
                });
                RTL.WriteOutput(OutputSource, "};");
                // Language
                ...
                RTL.WriteOutput(OutputSource, SymbolDef + " = { " + string.Join(", ", Members) + " };\r\n");
            }
        }
        public static DateTime GetLastModifyTime(string ProjectDir, CO_I18N ConfigObject) {
            ...
            foreach (var I18NConfFile in I18NConfFiles) {
                DateTime SourceModifyTime = File.GetLastWriteTime(I18NConfFile);
        ...
        public static bool Process(string ProjectDir, FileStream fsHeader, FileStream fsSource, CO_I18N ConfigObject) {
            ...
            // Compile
            RTL.WriteOutput(fsHeader, "#define Precomp4C_I18N_ITEM_COUNT " + I18NItems.Count.ToString() + "\r\n");
            RTL.WriteOutput(fsHeader, "typedef enum _I18N_ITEM_INDEX {");
            I18NItems.ForEach(I18NItem => {
                RTL.WriteOutput(fsHeader, "\tI18NIndex_" + I18NItem.Key + (I18NItem == I18NItems[^1] ? string.Empty : ","));
            ...
            RTL.WriteOutput(fsHeader, "} I18N_ITEM_INDEX, *PI18N_ITEM_INDEX;\r\n");
            // Output languages
            ...
            string SymbolDef = string.Format("PI18N_LANGUAGE Precomp4C_I18N_Lang_Table[{0:D}]", I18NLangs.Count);
            RTL.WriteOutput(fsHeader, "extern " + SymbolDef + ";\r\n");
            RTL.WriteOutput(fsSource, SymbolDef + " = { " + string.Join(", ", LangSymbols) + " };\r\n");
