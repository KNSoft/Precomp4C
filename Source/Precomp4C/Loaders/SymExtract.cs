...
namespace Precomp4C.Loaders {
    static class SymExtract {
        ...
        public class SymExtractSection {
            ...
            public List<SymExtractSymbol> Symbols = new();
        ...
        public class SymExtractFile {
            ...
            public List<SymExtractSection> Sections = new();
        ...
        public static DateTime GetLastModifyTime(string ProjectDir, CO_SymbolExtract[] ConfigObject) {
            ...
            foreach (var SymbolItem in ConfigObject) {
                ...
                DateTime SourceModifyTime = File.GetLastWriteTime(SymFilePath);
        ...
        public static bool Process(string ProjectDir, FileStream fsHeader, FileStream fsSource, CO_SymbolExtract[] ConfigObject) {
            Stopwatch Elapsed = new();
            Elapsed.Start();
            Console.WriteLine("SymExtract Loader starts...");

            // Process each file
            foreach (var SymbolItem in ConfigObject) {
                SymExtractFile SymFile = new(ProjectDir, SymbolItem);
                foreach (var Symbol in SymbolItem.Symbols) {
                    ...
                    foreach (var Section in SymFile.Sections) {
                        ...
                        // Write to output
                        var SymbolDef = string.Format("unsigned char {0}[{1:D}]", "SYM_" + (string.IsNullOrWhiteSpace(Symbol.Export) ? Symbol.Name : Symbol.Export), Size);
            ...
            Elapsed.Stop();
            Console.WriteLine("SymExtract Loader finished, elapsed {0:G}ms", Elapsed.Elapsed.TotalMilliseconds);
            return true;
        }
    }
}
